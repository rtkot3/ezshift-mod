(function() {
    ...
    const UI = {
        ...
        injectLayout() {
            ...
            if (sidebar) {
                ...
                <div class="sidebar-section ms-3 mx-3 mt-2" style="margin-bottom: 20px">
                  <div class="mb-2">
                    <span class="fw-semibold fs-lg p-0">EZShift r3 Mod</span>
                  ...
                  <div style="display: none" id="r3_hidden_block">
                    ...
                    <div class="mt-2">
                      <span class="fw-semibold fs-lg p-0">Find Schedule</span>
                    </div>
                    ...
                    <div class="mt-2">
                      ...
                      <div style="" id="r3_find_imposter_block">
                          ...
                          <div class="mt-2 d-flex align-items-center">
                              ...
                              <select disabled id="r3_imposters_date_select" class="text-dark bg-light border p-2" style="width:100%"> 
                                  <option value="">No Data</option> 
                              </select>
                          </div>
                          ...
                          <div class="mt-2 d-flex align-items-center">
                              ...
                              <select disabled id="r3_imposters_area_select" class="text-dark bg-light border p-2" style="width:100%"> 
                                  <option value="">No Data</option> 
                              </select>
                          </div> 
                  
                          <div class="btn text-white mt-2" id="r3_imposters_find" style="background-color:#411F54;cursor:pointer; width: 100%">
                              Find<i class="ph-magnifying-glass RotateIconForRTL ms-1"></i>
                          </div>

                          <div class="mt-2">
                              <span class="fs-lg p-0">List of People:</span>
                          </div>
                      
                          <div id="r3_imposters_list" class="p-2 rounded mt-2 pb-1" style="background-color: #ebebeb">
                              <div class="mb-1">
                                  <span>Select Date &amp; Area and click Find</span>
                              </div>
                          </div>

                      </div>

                      <div style="" id="r3_find_сolleagues_block">

                          <div class="mt-2">

                              <span class="fw-semibold fs-lg p-0">Find Colleagues</span>
                              ...
                              <div class="dropdown float-end" style="padding-top:2px;">
                                  ...
                                  <div class="dropdown-menu dropdown-menu-end text-dark px-2">
                                      The ability to quickly find out what areas your teammates have.
                                  </div> 
                              </div>
                          ...
                          <div class="mt-2 d-flex align-items-center">
                              <span class="fs-lg p-0 me-2" style="white-space: nowrap;">Date:</span>
                              <select disabled id="r3_сolleagues_date_select" class="text-dark bg-light border p-2" style="width:100%"> 
                                  <option value="">No Data</option> 
                              </select>
                          </div>
                          ...
                          <div class="mt-2 d-flex align-items-center">
                              <span class="fs-lg p-0 me-2" style="white-space: nowrap;">Area:</span>
                              <select disabled id="r3_сolleagues_area_select" class="text-dark bg-light border p-2" style="width:100%"> 
                                  <option value="">No Data</option> 
                              </select>
                          </div> 
                      
                          <div class="btn text-white mt-2" id="r3_сolleagues_find" style="background-color:#411F54;cursor:pointer; width: 100%">
                              Find<i class="ph-magnifying-glass RotateIconForRTL ms-1"></i>
                          </div>
                          
                          <div class="mt-2">
                              <span class="fs-lg p-0">List of People:</span>
                              <span class="fw-semibold" id="r3_team_selected" style="color: inherit;">No Data</span>
                          </div>

                          <div id="r3_сolleagues_list" class="p-2 rounded mt-2 pb-1" style="background-color: #ebebeb">
                              <div class="mb-1">
                                  <span>Select Date &amp; Area and click Find</span>
                              </div>
                          </div>
        ...
        updateShiftsTitle(personName) {
          const titleSpans = document.querySelectorAll('.col-xl-6.col-12 .mb-2 > div > span.fw-semibold.fs-lg.p-0');
          for (let span of titleSpans) {
              if (span.innerText.includes('Shifts') || span.innerText.includes('shifts')) {
                  span.innerText = `${personName}'s shifts`;
                  break;
              }
          }
        },
    ...
    const API = {
        async fetchScheduleData(offset = 0) {
            ...
            try {
                ...
                const dateSel = document.querySelector('#r3_imposters_date_select');
                const areaSel = document.querySelector('#r3_imposters_area_select');
                if (dateSel && areaSel) {
                ...
                // Загружаем даты и в селектор коллег
                const collDateSel = document.querySelector('#r3_сolleagues_date_select');
                if (collDateSel) {
                    collDateSel.innerHTML = days.map(d => `<option value="${d.fullDate}">${d.fullDate}</option>`).join('');
                    collDateSel.disabled = false;
                }
    ...
    const Handlers = {
        async onGetData(e) {
            ...
            const uiEls = {
                ...
                teamSelected: document.querySelector('#r3_team_selected'),
                collList: document.querySelector('#r3_сolleagues_list')
            };
            ...
            if (uiEls.teamSelected) { uiEls.teamSelected.innerText = "No Data"; uiEls.teamSelected.style.color = "inherit"; }
            if (uiEls.collList) uiEls.collList.innerHTML = '<div class="mb-1"><span>No Data</span></div>';
            ...
            try {
                ...
                if (State.peopleData.length) {
                    ...
                    // Автоматически триггерим изменения дат для инициализации списков зон
                    document.querySelector('#r3_imposters_date_select')?.dispatchEvent(new Event('change'));
                    document.querySelector('#r3_сolleagues_date_select')?.dispatchEvent(new Event('change'));
        ...
        onPerformSearch() {
            ...
            UI.fixTableBorders();
            UI.updateShiftsTitle(selectedName);
        ...
        onFindImposters() {
          const selectedDate = document.querySelector('#r3_imposters_date_select')?.value;
          const selectedArea = document.querySelector('#r3_imposters_area_select')?.value;
          ...
          const shiftPriority = { "LNGT": 1, "NGT": 2, "LMNG": 3, "MNG": 4, "AFT": 5, "DPEAK": 6, "LAFT": 7, "PEAK": 8, "NPEAK": 9, "none": 10 };
      ...
      onImposterDateChange() {
        ...
        // --- ЛОГИКА ДЛЯ FIND COLLEAGUES (ИЗГНАННИКИ ИЗ СВОЕЙ AREA) ---
        onColleaguesDateChange() {
            const selectedDate = document.querySelector('#r3_сolleagues_date_select')?.value;
            const areaSel = document.querySelector('#r3_сolleagues_area_select');
            if (!selectedDate || !areaSel || !State.peopleData.length) return;

            const activeAreas = new Set();

            State.peopleData.forEach(person => {
                // Вычисляем родную (Main) Area человека
                const counts = {};
                person.shifts.forEach(s => {
                    if (s.area !== 'SL' && s.area !== 'ABSENCE') {
                        counts[s.area] = (counts[s.area] || 0) + 1;
                ...
                // Смотрим его смену в этот день
                const dayShift = person.shifts.find(s => s.fullDate === selectedDate);
                // Если его отправили в ДРУГУЮ зону (не его основную) — значит для его родной зоны есть событие "свапа"!
                if (dayShift && dayShift.area !== 'SL' && dayShift.area !== 'ABSENCE' && dayShift.area !== mainArea) {
                    activeAreas.add(mainArea);
                }
            });
            ...
            if (activeAreas.size > 0) {
                areaSel.disabled = false;
                areaSel.innerHTML = Array.from(activeAreas).sort().map(a => `<option value="${a}">${a}</option>`).join('');
            } else {
                areaSel.disabled = true;
                areaSel.innerHTML = '<option value="">No Swaps...</option>';
            }

            const listContainer = document.querySelector('#r3_сolleagues_list');
            if (listContainer) listContainer.innerHTML = '<div class="mb-1"><span>Select Date & Area and click Find</span></div>';
            
            const teamSelected = document.querySelector('#r3_team_selected');
            if (teamSelected) {
                teamSelected.innerText = "No Data";
                teamSelected.style.color = "inherit";
            }
        },

        onFindColleagues() {
            const selectedDate = document.querySelector('#r3_сolleagues_date_select')?.value;
            const selectedArea = document.querySelector('#r3_сolleagues_area_select')?.value;
            const listContainer = document.querySelector('#r3_сolleagues_list');
            const teamSelected = document.querySelector('#r3_team_selected');
            ...
            let colleagues = [];
            let selectedAreaColor = "inherit";

            State.peopleData.forEach(person => {
                // Вычисляем родную Area
                const counts = {};
                person.shifts.forEach(s => {
                    if (s.area !== 'SL' && s.area !== 'ABSENCE') {
                        if (!counts[s.area]) counts[s.area] = { count: 0, color: s.color };
                        counts[s.area].count++;
                    }
                });
                let mainArea = "";
                let max = 0;
                let mainColor = "inherit";
                for (const a in counts) {
                    if (counts[a].count > max) {
                        max = counts[a].count;
                        mainArea = a;
                        mainColor = counts[a].color;
                    }
                }

                // Если человек НАШ (его родная зона совпадает с выбранной)
                if (mainArea === selectedArea) {
                    // Захватываем базовый цвет нашей зоны, чтобы раскрасить заголовок
                    if (selectedAreaColor === "inherit" && mainColor !== "inherit") {
                        selectedAreaColor = Utils.darkenColor(mainColor, CONFIG.darkness);
                    }

                    // Проверяем, где он сегодня работает
                    const currentShift = person.shifts.find(s => s.fullDate === selectedDate);
                    // Если работает в чужой зоне
                    if (currentShift && currentShift.area !== 'SL' && currentShift.area !== 'ABSENCE' && currentShift.area !== selectedArea) {
                        colleagues.push({
                            name: person.name,
                            shiftType: currentShift.shiftType || 'none',
                            targetArea: currentShift.area, // Куда его "выгнали" (например, AURORA)
                            targetColor: Utils.darkenColor(currentShift.color, CONFIG.darkness) // Цвет чужой зоны
                        });
                    }
                }
            });

            // Сортировка по типу смены
            colleagues.sort((a, b) => {
                const priorityA = shiftPriority[a.shiftType] || 99;
                const priorityB = shiftPriority[b.shiftType] || 99;
                return priorityA - priorityB;
            });

            // Меняем текст «No Data» сверху списка на имя выбранной родной Area
            if (teamSelected) {
                teamSelected.innerText = selectedArea;
                teamSelected.style.color = selectedAreaColor;
            }

            // Отрисовка списка
            if (colleagues.length === 0) {
                listContainer.innerHTML = '<div class="mb-1"><span>No Swapped Colleagues...</span></div>';
            } else {
                colleagues.forEach(coll => {
                    const typeText = coll.shiftType !== 'none' ? coll.shiftType : '';
                    ...
                    row.innerHTML = `
                        <span style="max-width: 170px; text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">
                            <div class="text-white badge me-2 r3-goto-person" style="border: 2px solid #411f54; background-color: #411f54; cursor: pointer;" data-name="${coll.name}">
                              <span>&gt;</span>
                            </div>${coll.name}
                        </span>
                        <div class="d-flex ps-2" style="flex-direction: column;align-items: end;">
                            <span class="fw-semibold" style="font-size:10px; letter-spacing: 0.5px;">${typeText}</span>
                            <span class="fw-semibold" style="color: ${coll.targetColor};">${coll.targetArea}</span>
                        </div>
                    `;
                    listContainer.appendChild(row);
                });

                // Подключаем кнопки быстрого перехода к графику человека по клику на стрелочку
                listContainer.querySelectorAll('.r3-goto-person').forEach(btn => {
                    btn.addEventListener('click', Handlers.onGotoPerson);
                });
            }
    ...
    function initializeScript() {
        ...
        document.querySelector('#r3_imposters_find')?.addEventListener('click', Handlers.onFindImposters);
        document.querySelector('#r3_imposters_date_select')?.addEventListener('change', Handlers.onImposterDateChange);
        
        // Новые слушатели для Find Colleagues
        document.querySelector('#r3_сolleagues_find')?.addEventListener('click', Handlers.onFindColleagues);
        document.querySelector('#r3_сolleagues_date_select')?.addEventListener('change', Handlers.onColleaguesDateChange);
